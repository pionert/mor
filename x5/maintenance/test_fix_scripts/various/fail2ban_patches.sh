...
mkdir -p /usr/local/mor/backups/fail2ban/$TIMESTAMP
cp -fr /etc/fail2ban/* /usr/local/mor/backups/fail2ban/$TIMESTAMP
...
if [ "$STATUS" == "0" ]; then
    ...
    check_if_ssh_logs_failures_to_messages
    w00tfix
#    disable_apache_phpmyadmin   #commented because of trac #10658
    #===================
...
# TODO fix this, add some checks, why overwrite all the time?
...
# TODO why some many functions in this file and only this one is executed?
patch_fail2ban_v0_8_4

# and restart  again
# We do not need to restart but it doesnt  hurt too much either, MK: BS

#MK /etc/init.d/fail2ban restart
