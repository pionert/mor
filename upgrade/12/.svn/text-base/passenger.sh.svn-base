...
passenger_gem()
{
    ...
    if [ "$passenger_gem" == "1" ] && [ -f /etc/httpd/conf.d/mor.conf ] && [ -f /etc/httpd/conf.d/passenger.conf ] && [ -f `grep mod_passenger.so /etc/httpd/conf.d/passenger.conf  | awk '{print $NF}'` ]; then
...
#===== Main ==================
RESTART_REQUIRED=0
passenger_gem ruby-1.9.3-p194 12
check_if_name_virtual_host_option_enabled
migrate_mor_apache_config ruby-1.9.3-p194 12
