...
if [ "$?" != "1" ]; then    #if core is present in the system
    ...
    elif [ "$MOR_CORE_BRANCH" -ge "12" ] ; then
        NEWEST_ASTERISK_VERSION=`grep ASTERISK_VER /usr/src/mor/sh_scripts/asterisk/install_asterisk.sh | awk -F"=" '{print $2}' | awk -F"\"" '{print $2}' | head -n 1`
    else
        NEWEST_ASTERISK_VERSION="1.4.42"
    fi
