...
ASTERISK_VER="1.8.23.0"

# temporary
yum -y install glibc tar mysql-devel gcc gcc-c++ ncurses-devel bison openssl openssl-devel gnutls gnutls-devel zlib-devel ghostscript make subversion wget sox chkconfig vixie-cron which logrotate postfix lynx gzip bc libxml2-devel flex patch autoconf automake libtool  openldap-devel
...
#proper init script copying if we upgrade from ast 1.4
/bin/cp -r /usr/src/mor/sh_scripts/asterisk/asterisk /etc/init.d


# Fix internal Asterisk DB
cp -fr /usr/src/mor/sh_scripts/asterisk/db/astdb_1_8 /var/lib/asterisk/astdb
...
/bin/cp -fr /usr/src/mor/asterisk-conf/ast_1.8/cdr.conf /etc/asterisk
