...
check_if_percona_upgrade_is_needed()
{
    if [ `/usr/bin/mysql -h "$DB_HOST" -u $DB_USERNAME --password=$DB_PASSWORD "$DB_NAME" -e "desc calls;" | grep dcontext | wc -l` == "1" ]; then
        ...
        /usr/src/mor/upgrade/x4/live_very_big_db_migration.sh "$NO_SCREEN"
        if [ "$?" != "0" ]; then
            exit 1
        fi
    fi
...
FILE="/usr/src/mor/db/x4/beta_structure.sql"
exec < $FILE
while read LINE
do
  mysql_sql "$LINE"
...
/usr/bin/mysql -h "$DB_HOST" -u $DB_USERNAME --password=$DB_PASSWORD "$DB_NAME" < /usr/src/mor/sh_scripts/asterisk/db/triggers.sql

echo "Triggers updated"

echo "Importing DATA Changes"

/usr/bin/mysql -h "$DB_HOST" -u $DB_USERNAME --password=$DB_PASSWORD "$DB_NAME" < /usr/src/mor/db/x4/beta_data.sql

echo "Data changes import complete"


echo "Importing Currency Changes"
/usr/bin/mysql -h "$DB_HOST" -u $DB_USERNAME --password=$DB_PASSWORD "$DB_NAME" < /usr/src/mor/db/x4/currency_changes.sql
...
/usr/bin/mysql -h "$DB_HOST" -u "$DB_USERNAME" --password="$DB_PASSWORD" "$DB_NAME" < /usr/src/mor/db/x4/permissions.sql    


echo "Permission import complete"

/usr/src/mor/upgrade/x4/percona_call_details_index.sh "$NO_SCREEN"