...
int main(int argc, char *argv[]) {
    ...
    mor_init("Starting 'MOR X6 CDR Rerate' script\n");

    if (mor_task_get(1, &user_id, &owner_id, date_from, date_till, ghost_time_str, include_reseller_users_str, NULL, NULL)) return 1;
...
void *get_rate_details(void *arg) {
    ...
    for (user = 1; user <= 3; user++) {
        ...
        if (tariff_type == 0) {
            ...
            sprintf(sqlcmd, "SELECT prefix, rate, increment_s, min_time, connection_fee "
                "FROM rates "
                "JOIN ratedetails ON (rate_id = rates.id AND (daytype = '%s' OR daytype = '') AND '%s' BETWEEN start_time AND end_time) "
                "WHERE prefix IN (%s) AND tariff_id = %i AND (effective_from <= '%s' OR effective_from IS NULL) "
                "ORDER BY LENGTH(rates.prefix) DESC, rates.effective_from DESC LIMIT 1",
                daytype, call_time, prefix_sql_line, tariff_id, calldate);
            ...
            *total_event_price = 0;
            ...
            sprintf(sqlcmd, "SELECT B.prefix, aid, afrom, adur, atype, around, aprice, acid, acfrom, acdur, actype, acround, acprice, ghostminperc "
                "FROM (SELECT A.prefix, aratedetails.id as 'aid', aratedetails.from as 'afrom', aratedetails.duration as 'adur', aratedetails.artype as 'atype', "
                "aratedetails.round as 'around', aratedetails.price as 'aprice', acustratedetails.id as 'acid', acustratedetails.from as 'acfrom', "
                "acustratedetails.duration as 'acdur', acustratedetails.artype as 'actype', acustratedetails.round as 'acround', acustratedetails.price as 'acprice', "
                "SUM(acustratedetails.id) as 'sacid', rates.ghost_min_perc as 'ghostminperc' "
                "FROM rates "
                "LEFT JOIN aratedetails ON (aratedetails.rate_id = rates.id  AND '%s' BETWEEN aratedetails.start_time AND aratedetails.end_time AND (aratedetails.daytype = '%s' OR aratedetails.daytype = '')) "
                "JOIN destinationgroups ON (destinationgroups.id = rates.destinationgroup_id) "
                "JOIN (SELECT destinations.* FROM  destinations WHERE destinations.destinationgroup_id != 0 AND destinations.prefix IN (%s) ORDER BY LENGTH(destinations.prefix) DESC LIMIT 1) as A ON (A.destinationgroup_id = destinationgroups.id) "
                "LEFT JOIN customrates ON (customrates.destinationgroup_id = destinationgroups.id AND customrates.user_id = %i) "
                "LEFT JOIN acustratedetails ON (acustratedetails.customrate_id = customrates.id AND '%s' BETWEEN acustratedetails.start_time AND acustratedetails.end_time AND (acustratedetails.daytype = '%s' OR acustratedetails.daytype = '')) "
                "WHERE rates.tariff_id = %i GROUP BY aratedetails.id, acustratedetails.id ) AS B "
                "GROUP BY IF(B.sacid > 0,B.acid,B.aid) "
                "ORDER BY acfrom ASC, actype ASC, afrom ASC, atype ASC",
                call_time, daytype, prefix_sql_line, user_id, call_time, daytype, tariff_id);
