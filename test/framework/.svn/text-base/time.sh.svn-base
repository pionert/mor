...
get_timestamp_plus_minutes()
{
    ...
    #   Year:   2013
    #   About:  This method gives unix timestamp + minutes defined by 2nd parameter
    #
    #   Arguments
    #       $1  -   date to convert to timestamp
    #       $2  -   add this amount of hours
    
    local date_to_operate_with="$1"
    local minutes_to_add="$2"
    
    date_in_timestamp=`date --utc --date "$date_to_operate_with" +%s`
    
    if [ ! -f /usr/bin/bc ]; then   #not present by default
        yum -y install bc;
    fi
    echo "$date_in_timestamp + $minutes_to_add*60" | bc
}
# USAGE # # # # # # # # # # # # # # # # # # # # # # # # # # # # # 
