...
#define NEW_CONFPATH  "/etc/mor/system.conf"
...
// nice log
#define mor_log(M, ...) {char mor_log_buffer[10000] = ""; sprintf(mor_log_buffer, M, ##__VA_ARGS__); _mor_log(mor_log_buffer, 0); mor_log_buffer[0] = 0;}
#define mor_log_header(M, ...) {char mor_log_buffer[10000] = ""; sprintf(mor_log_buffer, M, ##__VA_ARGS__); _mor_log(mor_log_buffer, 1); mor_log_buffer[0] = 0;}
...
int mor_get_variable(char *variable, char *value) {
    ...
    file = fopen(NEW_CONFPATH, "r");

    if (!file) {
        mor_log("Cannot read configuration variables from: " NEW_CONFPATH "\n");
        return 1;
    }
    ...
    if (found) {
        mor_log("Variable: %s = %s\n", variable, value);
    } else {
        mor_log("Variable '%s' was not found in " NEW_CONFPATH "\n", variable);
    }
