...
. /usr/src/mor/x5/framework/settings.sh
   ...
   if [ ! -n "$1" ] && [ ! -n "$2" ]; then
    ...
    if [ "$?" != "0" ]; then
        echo "Failed to make a backup of current DB before restoring. DEBUG: $1 $_BACKUP_FOLDER $OUT" >> $LOG_FILE
        echo 1
        exit 1
    ...
    if [ -f "$_BACKUP_FOLDER"/db_dump_"$_DATE".sql.tar.gz ]; then
        ...
        if [ "$?" == "0" ]; then
            echo Extracted backup "$_BACKUP_FOLDER"/db_dump_$_DATE.sql.tar.gz >> $LOG_FILE
        else
            echo Failed to extract backup "$_BACKUP_FOLDER"/db_dump_"$_DATE".sql.tar.gz >> $LOG_FILE
            echo 1
            exit 1
        fi
    else
        echo Backup file does not exist: "$_BACKUP_FOLDER"/db_dump_"$_DATE".sql.tar.gz >> $LOG_FILE
        echo 1
        exit 1
    fi
    ...
    if [ "$?" == "0" ]; then
        echo Successfully imported backup "$_BACKUP_FOLDER"/db_dump_"$_DATE".sql.tar.gz >> $LOG_FILE
    else
        echo "Failed to import backup" >> $LOG_FILE
        echo 1
        exit 1
    fi
