...
class Poster
  ...
  def initialize(version, revision, test, status, node_request_log_id, node_received, node_selenium_start, node_selenium_end, files_args = [])
    puts "initialize(version = #{version}, revision = #{revision}, test = #{test}, status = #{status}, node_received = #{node_received}, node_selenium_start = #{node_selenium_start}, node_selenium_end = #{node_selenium_end})"
    ...
    @node_request_log_id =  node_request_log_id
    @node_received = node_received
    @node_selenium_start = node_selenium_start
    @node_selenium_end = node_selenium_end
    @files = {
      :my_debug => "",
      :crash_log=> "",
      :production_log=> "",
  ...
  def send_request
    if @debug
      ...
      puts "  >> IP : #{@ip}"
      puts "  >> Node request log ID : #{@node_request_log_id}"
    end
    request_hash = {
      :version => @version,
      :revision => @revision,
...
if ARGV.size < 8 or !["OK", "FAILED"].include?(ARGV[3])
  puts @help_text
else
  puts "OK #{ARGV.inspect}"

  p = Poster.new(ARGV[0], ARGV[1], ARGV[2], ARGV[3], ARGV[4], ARGV[5], ARGV[6], ARGV[7], ARGV[8..-1])
  p.send_request()
end
