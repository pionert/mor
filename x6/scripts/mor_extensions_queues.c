...
int main() {
    ...
    // get all queues
    sprintf(query, "SELECT queues.id, queues.id, queues.name, max_wait_time, ivr_voices.voice, ivr_sound_files.path, queues.allow_caller_hangup, queues.allow_callee_hangup, "
        "queues.ringing_instead_of_moh, queues.ring_at_once, queues.failover_action, queues.failover_data, dids.did, devices.name, queues.pbx_pool_id, users.pbx_pool_id FROM queues "
    ...
    while (( row = mysql_fetch_row(result) )) {
        ...
        if (row[10]) {
            ...
            if (row[11] && row[14] && strcmp(row[10], "extension") == 0) {
                if (atoi(row[14]) > 1) {
                    printf("exten => s,n,Goto(pool_%s_mor_local,%s,1)\n", row[14], row[11]);
                } else {
                    printf("exten => s,n,Goto(mor_local,%s,1)\n", row[11]);
                }
            }
            if (row[12] && row[14] && strcmp(row[10], "did") == 0) {
                if (atoi(row[14]) > 1) {
                    printf("exten => s,n,Goto(pool_%s_mor_local,%s,1)\n", row[14], row[12]);
                } else {
                    printf("exten => s,n,Goto(mor_local,%s,1)\n", row[12]);
            ...
            if (row[13] && row[15] && strcmp(row[10], "device") == 0) {
                if (atoi(row[15]) > 1) {
                    printf("exten => s,n,Goto(pool_%s_mor_local,%s,1)\n", row[15], row[13]);
                } else {
                    printf("exten => s,n,Goto(mor_local,%s,1)\n", row[13]);
