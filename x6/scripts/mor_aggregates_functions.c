...
int comp_by_dst_user(const void * elem1, const void * elem2) {
    calls_data_t *f = (calls_data_t *)elem1;
    calls_data_t *s = (calls_data_t *)elem2;
    if (f->dst_user_id > s->dst_user_id) return  1;
    if (f->dst_user_id < s->dst_user_id) return -1;
    return 0;
}
...
void *mor_aggregate(void *args) {
    ...
    if (type == 12) {
        qsort(calls, calls_data_count, sizeof(calls_data_t), comp_by_dst_user);
    }
    if (type == 13) {
        qsort(calls, calls_data_count, sizeof(calls_data_t), comp_by_originator);
    }
    ...
    for (i = 0; i < calls_data_count; i++) {
        ...
        if ((type == 2 || type == 3 || type == 6 || type == 7 || type == 10 || type == 12 || type == 13) && (calls[i].reseller || calls[i].partner)) {
            goto skip;
        }

        if ((type == 12 || type == 13) && calls[i].did_id == 0) {
            goto skip;
        }
        ...
        // *SPECIAL* AGGREGATE BY DID OWNER PRICE
        if (type == 12) {
            if (!calls[i].dst_user_id > 0) goto skip;
            if ((user_id != calls[i].dst_user_id || tphid != calls[i].time_period_hour_id) && count) got_data = 1;
        }

        // *SPECIAL* AGGREGATE BY DID INCOMING PRICE
        if (type == 13) {
            if (!calls[i].user_id > 0) goto skip;
            if ((user_id != calls[i].user_id || tphid != calls[i].time_period_hour_id) && count) got_data = 1;
        }
        ...
        if (calls[i].answered) {
            answered++;
            if (type == 12) {
                user_price += calls[i].did_price;
                user_price_tax += calls[i].did_price_with_tax;
                user_billsec += calls[i].did_billsec;
            } else if (type == 13) {
                user_price += calls[i].did_inc_price;
                user_price_tax += calls[i].did_inc_price_with_tax;
                user_billsec += calls[i].did_billsec;
            } else {
                if (calls[i].reseller == 0 && calls[i].partner == 0) {
                    user_price += calls[i].user_price;
                    user_price_tax += calls[i].user_price_with_tax;
                    user_billsec += calls[i].user_billsec;
                } else if (calls[i].reseller == 1) {
                    user_price += calls[i].reseller_price;
                    user_price_tax += calls[i].reseller_price_with_tax;
                    user_billsec += calls[i].reseller_billsec;
                } else if (calls[i].partner == 1) {
                    user_price += calls[i].partner_price;
                    user_price_tax += calls[i].partner_price_with_tax;
                    user_billsec += calls[i].partner_billsec;
                }
            }
        ...
        if (type == 1 || type == 4 || type == 5 || type == 8 || type == 9 || type == 11 || type == 13) user_id = calls[i].user_id;
