...
install_mysql_5_5()
{
    ...
    _centos_version
    if [ "$centos_version" == "5" ]; then
        yum -y --nogpgcheck --enablerepo=remi,remi-test install mysql mysql-server mysql-devel
    else    # centos 6
        yum -y --nogpgcheck --releasever=6 --enablerepo=remi,remi-test install mysql mysql-server mysql-devel
    fi
    if [ "$?" == "0" ]; then
...
upgrade_tables
# reconfigure mysql
sh /usr/src/mor/x5/mysql/configure_mycnf.sh
