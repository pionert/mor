...
configure_zabbix_agentd()
{
    ...
    if [ "$?" == "0" ]; then    
        sed -i '/Include=\/usr\/src\/mor\/test\/zabbix\/mor/d' /etc/zabbix/zabbix_agentd.conf
        awk -F"#" '{print $1}' /etc/zabbix/zabbix_agentd.conf | sed "s/ //g" | grep "Include=/usr/src/mor/test/zabbix/mor" &> /dev/null
        if [ "$?" == "0" ]; then
            report "Zabbix configuration: old include was not removed" 1
        else
            report "Zabbix configuration: old include was removed" 4
            /etc/init.d/zabbix-agent restart
        fi
    fi
    
    awk -F"#" '{print $1}' /etc/zabbix/zabbix_agentd.conf | sed "s/ //g" | grep "Include=/usr/src/mor/x6/helpers/zabbix/mor" &> /dev/null
    if [ "$?" != "0" ]; then    
        echo "Include=/usr/src/mor/x6/helpers/zabbix/mor" >> /etc/zabbix/zabbix_agentd.conf 
        /etc/init.d/zabbix-agent restart
        #----Checking again
		awk -F"#" '{print $1}' /etc/zabbix/zabbix_agentd.conf | sed "s/ //g" | grep "Include=/usr/src/mor/x6/helpers/zabbix/mor" &> /dev/null
        if [ "$?" == "0" ]; then    
