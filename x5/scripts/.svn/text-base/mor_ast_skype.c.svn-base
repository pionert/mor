/*
*
*	MOR Asterisk Skype configuration generation script
*	Copyright Mindaugas Kezys / Kolmisoft 2011
*
*	v0.1
*
*	2011-03-15 0.1 Initial version
*
*/
...
int generate_skype_general_section();
int generate_skype_device_configuration();
...
void my_debug(char *msg) {
    FILE *file;
    file = fopen("/var/log/mor/mor_ast_skype.log","a+");
    fprintf(file,"%s\n",msg);
    fclose(file);
}
...
main(int argc, char *argv[]) {
    ...
    generate_skype_general_section();
    
    generate_skype_device_configuration();    
...
//        printf("\n; ============= MOR Skype configuration ============\n\n; DO NOT EDIT THIS FILE!\n\n; IT IS GENERATED AUTOMATICALLY BY MOR.\n\n; ==================================================\n\n\n");

	printf("[general]\n");


	sprintf(sqlcmd,"SELECT (SELECT login FROM providers JOIN conflines ON (providers.id = value)  WHERE conflines.name = 'Skype_Default_Provider'),	(SELECT login FROM providers WHERE tech = 'Skype'  ORDER BY id ASC LIMIT 1);");
//        my_debug(sqlcmd);
...
//        my_debug(sqlcmd);
	...
	else // query succeeded, process any data returned by it
	{
		...
		while ((row = mysql_fetch_row(result)))
		{
		    ...
		    sprintf(buff, "%scontext=mor\n", buff);
		    
		    
		    //incomming extension 
		    if ((row[29]) && (strlen(row[29]))) {
			sprintf(buff, "%sexten=%s\n", buff, row[29]);
		    } else {
			sprintf(buff, "%sexten=s\n", buff);    
		    }
