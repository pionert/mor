...
configure_zabbix_agentd()
{
    ...
    awk -F"#" '{print $1}' /etc/zabbix/zabbix_agentd.conf | sed "s/ //g" | grep "Include=/usr/src/mor/x5/helpers/zabbix/mor" &> /dev/null
    if [ "$?" != "0" ]; then    
        echo "Include=/usr/src/mor/x5/helpers/zabbix/mor" >> /etc/zabbix/zabbix_agentd.conf 
        /etc/init.d/zabbix-agent restart
        #----Checking again
		awk -F"#" '{print $1}' /etc/zabbix/zabbix_agentd.conf | sed "s/ //g" | grep "Include=/usr/src/mor/x5/helpers/zabbix/mor" &> /dev/null
        if [ "$?" == "0" ]; then    
