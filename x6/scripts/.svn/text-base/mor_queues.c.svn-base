...
int main() {
    ...
    while (( row = mysql_fetch_row(result) )) {
        ...
        sprintf(members_query, "SELECT devices.extension, devices.context, penalty FROM queue_agents "
        ...
        while (( members_row = mysql_fetch_row(members_result) )) {
            if (members_row[0] && members_row[1] && members_row[2]) {
                printf("member => Local/%s@%s,%s\n", members_row[0], members_row[1], members_row[2]);
            }
        }
