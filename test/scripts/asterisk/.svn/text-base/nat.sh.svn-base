...
check_sip_conf_for_nat_settings()
{
    ...
    if [ "$externip" == "0" ] && [ "$externhost" == "0" ]; then
        report "NAT detected and you have not set externip neither externhost in /etc/asterisk/sip.conf" 3
    fi

    local localnet=`awk -F";" '{ print $1}' /etc/asterisk/sip.conf | grep localnet | wc -l`
    if [ "$localnet" == "0" ]; then
        report "NAT detected and you have not set localnet in /etc/asterisk/sip.conf" 3
    fi
