...
#export PATH="/usr/kerberos/sbin:/usr/kerberos/bin:/usr/local/sbin:/usr/local/bin:/sbin:/bin:/usr/sbin:/usr/bin:/root/bin" # with this one ruby is not reachable
...
ruby /usr/src/mor/x5/framework/which_version_is_bigger.rb "5.0.9" $_mysql_version
if [ "$?" == "1" ]; then
