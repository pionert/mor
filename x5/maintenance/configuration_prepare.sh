...
import_db_settings() {
    ...
    #Jei skriptas kvieciamas is x5/install.sh,  /etc/asterisk/mor.conf dar nebus sukurtas 
...
important_component_settings() {
   ...
   # This function is invoked during x4 --> x5 upgrade
   # In X5 components configuration is stored in /etc/mor/system.conf
   # In X4 components configuration is stored in /etc/mor/{asterisk,gui,db}.conf files
   # So this function updates X5 components configuration files from X4 compoenents configuration files 
...
if [ -f /etc/mor/system.conf ]; then
  ...
  # import needed only on first x4->x5 update.sh, if we doing fresh x5/install.sh
