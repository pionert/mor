...
#define SCRIPT_NAME     "mor_background_tasks"

#include "mor_functions.c"

// path to background_scripts
#define PATH_TO_ARCHIVE_OLD_CALLS    "/usr/local/mor/mor_archive_old_calls"
#define PATH_TO_CDR_RERATE           "/usr/local/mor/mor_cdr_rerate"
#define PATH_TO_INVOICES             "/usr/local/mor/mor_invoices"
#define PATH_TO_INVOICES_RECALCULATE "/usr/local/mor/mor_invoices recalculate"
...
int main(int argc, char *argv[]) {
    ...
    // check if mor_cdr_rerate script should be executed
    if (check_cdr_rerate() == 0) {
        mor_log("Executing script " PATH_TO_CDR_RERATE "\n");
        system(PATH_TO_CDR_RERATE);
        return 0;
    }

    // check if mor_invoices script should be executed
    int status = check_invoices();
    if (status == 0 || status == -1) {
        if (status == 0) {
            mor_log("Executing script " PATH_TO_INVOICES "\n");
            system(PATH_TO_INVOICES);
        } else {
            mor_log("Executing script " PATH_TO_INVOICES_RECALCULATE "\n");
            system(PATH_TO_INVOICES_RECALCULATE);
        }
        return 0;
    }
...
int check_archive_old_calls() {
    ...
    if (archive_at_h == -1) {
        mor_log("[mor_archive_old_calls] Archive_at = -1. Script will not be executed...\n");
        return 1;
    }

    if (older_than == 0) {
        mor_log("[mor_archive_old_calls] Move_to_old_calls_older_than = 0. Script will not be executed...\n");
        return 1;
    }
    ...
    if (active_tasks > 0) {
        mor_log("[mor_archive_old_calls] Active_tasks = %d. Script will not be executed...\n", active_tasks);
        return 1;
    }
    ...
    if (recently_finished) {
        mor_log("[mor_archive_old_calls] Another task can not be executed within 5 minutes. Script will not be executed...\n");
        return 1;
    }
...
int check_cdr_rerate() {
    ...
    // check if task exists
    if (mor_mysql_query("SELECT (SELECT count(id) FROM background_tasks WHERE task_id = 1 AND status = 'WAITING'), (SELECT count(id) FROM background_tasks WHERE task_id = 1 AND status = 'IN PROGRESS' AND created_at > DATE_SUB(NOW(), INTERVAL 1 HOUR))")) {
        return 1;
    }
    ...
    if (active_tasks > 0) {
        mor_log("[mor_cdr_rerate] Active_tasks = %d. Script will not be executed...\n", active_tasks);
        return 1;
    }

    if (waiting_tasks == 0) {
        mor_log("[mor_cdr_rerate] Waiting_tasks = %d. Script will not be executed...\n", waiting_tasks);
        return 1;
    }
...
int check_invoices() {
    ...
    mor_log("Checking Invoices...\n");

    MYSQL_RES *result;
    MYSQL_RES *result_recalculate;
    MYSQL_ROW row;
    MYSQL_ROW row_recalculate;
    int active_tasks = 0;
    int active_tasks_recalculate = 0;
    int waiting_tasks = 0;
    int waiting_tasks_recalculate = 0;

    // check if task exists
    if (mor_mysql_query("SELECT (SELECT count(id) FROM background_tasks WHERE task_id = 3 AND status = 'WAITING'), (SELECT count(id) FROM background_tasks WHERE task_id = 3 AND status = 'IN PROGRESS' AND created_at > DATE_SUB(NOW(), INTERVAL 1 HOUR))")) {
        return 1;
    }
    ...
    // check if task exists
    if (mor_mysql_query("SELECT (SELECT count(id) FROM background_tasks WHERE task_id = 4 AND status = 'WAITING'), (SELECT count(id) FROM background_tasks WHERE task_id = 4 AND status = 'IN PROGRESS' AND created_at > DATE_SUB(NOW(), INTERVAL 1 HOUR))")) {
        return 1;
    }

    result_recalculate = mysql_store_result(&mysql);

    // fetch data
    while ((row_recalculate = mysql_fetch_row(result_recalculate)) != NULL) {
        if (row_recalculate[0]) waiting_tasks_recalculate = atoi(row_recalculate[0]);
        if (row_recalculate[1]) active_tasks_recalculate  = atoi(row_recalculate[1]);
    }

    mysql_free_result(result_recalculate);

    if (active_tasks > 0 || active_tasks_recalculate > 0) {
        mor_log("[mor_invoices] Active_tasks = %d. Script will not be executed...\n", active_tasks + active_tasks_recalculate);
        return 1;
    }

    if (waiting_tasks == 0 && waiting_tasks_recalculate == 0) {
        mor_log("[mor_invoices] Waiting_tasks = 0. Script will not be executed...\n");
        return 1;
    }
