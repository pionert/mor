...
#define CONFPATH           "/etc/mor/system.conf"
...
// nice log
#define mor_log(M, ...) {char mor_log_buffer[4096] = ""; snprintf(mor_log_buffer, 4090, M, ##__VA_ARGS__); _mor_log(mor_log_buffer, 0); mor_log_buffer[0] = 0;}
#define mor_log_header(M, ...) {char mor_log_buffer[4096] = ""; snprintf(mor_log_buffer, 4090, M, ##__VA_ARGS__); _mor_log(mor_log_buffer, 1); mor_log_buffer[0] = 0;}
...
int mor_get_variable(char *variable, char *value);
void mor_apply_taxes(double *amount, int compound, int tax1, int tax2, int tax3, int tax4, double tax1_value, double tax2_value, double tax3_value, double tax4_value);

// for background tasks
int mor_task_lock();
int mor_task_unlock(int status);
int mor_task_get(int task, int *user_id, int *owner_id, char *data1, char *data2, char *data3, char *data4, char *data5, char *data6);
int mor_task_finish();
...
int mor_read_config() {
    ...
    while (fscanf(file, "%s = %s", var, val) != EOF) {
        ...
        if (!strcmp(var, "dbhost")) {
            strcpy(dbhost, val);
        }
        if (!strcmp(var, "dbname")) {
            strcpy(dbname, val);
        }
        if (!strcmp(var, "dbuser")) {
            strcpy(dbuser, val);
        }
        if (!strcmp(var, "dbsecret")) {
            strcpy(dbpass, val);
        }
        if (!strcmp(var, "dbport")) {
            dbport = atoi(val);
        }
...
int mor_mysql_query(const char *query) {
    ...
    if (mysql_query(&mysql, query)) {
        char mysql_error_msg[1024] = "";
        strcpy(mysql_error_msg, mysql_error(&mysql));

        // check if this is deadlock error: Deadlock found when trying to get lock; try restarting transaction
        // let's try to do what messages suggest - try to restart transaction in few seconds

        if (strstr(mysql_error_msg, "Deadlock found when trying to get lock")) {

#ifdef AGGREGATES_CONTROL_SCRIPT
            mor_log("Deadlock was found, will try to restart query in 10 seconds\n");
            sleep(10);
#else
            mor_log("Deadlock was found, will try to restart query in 3 seconds\n");
            sleep(3);
#endif
            ...
            if (retried >= max_retries) {
                mor_log("Tried to insert data %d times and all attempts failed. Aborting...\n", max_retries);
                mor_log("SQL ERROR: %s\n", mysql_error_msg);
                mor_log("SQL QUERY: %s\n", query);
                return 1;
            }
...
void mor_escape_string(char *string, char c) {
    ...
    for (i = 0; i < len; i++) {
        if (string[i] == c) {
            memmove(string + i + 1, string + i, strlen(string + i));
            string[i] = '\\';
            i++;
            len++;
        }
...
int mor_task_get(int task, int *user_id, int *owner_id, char *data1, char *data2, char *data3, char *data4, char *data5, char *data6) {
    ...
    sprintf(sqlcmd, "SELECT id, owner_id, user_id, data1, data2, data3, data4, data5, data6 FROM background_tasks WHERE task_id = %d AND status = 1 ORDER BY created_at LIMIT 1", task);
    ...
    if (mor_mysql_query(sqlcmd)) {
        ...
        if (result) {
            while ((row = mysql_fetch_row(result))) {
                ...
                if (row[1] && owner_id) *owner_id = atoi(row[1]);
                if (row[2] && user_id) *user_id = atoi(row[2]);
                if (row[3] && data1) strcpy(data1, row[3]);
                if (row[4] && data2) strcpy(data2, row[4]);
                if (row[5] && data3) strcpy(data3, row[5]);
                if (row[6] && data4) strcpy(data4, row[6]);
                if (row[7] && data5) strcpy(data5, row[7]);
                if (row[8] && data6) strcpy(data6, row[8]);
            }
        }
    ...
    mor_log("Task retrieved - id: %i, user_id: %i, owner_id: %d, data1: %s, data2: %s, data3: %s, data4: %s, data5: %s, data6: %s\n",
            task_id,
            user_id == NULL ? 0 : *user_id,
            owner_id == NULL ? 0 : *owner_id,
...
/*
    Get specific variable value from config file
*/
...
void mor_apply_taxes(double *amount, int compound, int tax1, int tax2, int tax3, int tax4, double tax1_value, double tax2_value, double tax3_value, double tax4_value) {
    ...
    if (compound) {
        if (tax1 && tax1_value != 0) *amount += (*amount * tax1_value / 100.0);
        if (tax2 && tax2_value != 0) *amount += (*amount * tax2_value / 100.0);
        if (tax3 && tax3_value != 0) *amount += (*amount * tax3_value / 100.0);
        if (tax4 && tax4_value != 0) *amount += (*amount * tax4_value / 100.0);
    } else {
        tax = 0;
        if (tax1 && tax1_value != 0) tax += (*amount * tax1_value / 100.0);
        if (tax2 && tax2_value != 0) tax += (*amount * tax2_value / 100.0);
        if (tax3 && tax3_value != 0) tax += (*amount * tax3_value / 100.0);
        if (tax4 && tax4_value != 0) tax += (*amount * tax4_value / 100.0);
    }
    *amount += tax;
