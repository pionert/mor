...
mysql_check_if_grant_is_present()
{
	...
	if [ `/usr/bin/mysql -h "$DB_HOST" -u $DB_USERNAME --password=$DB_PASSWORD "$DB_NAME" -e "SHOW GRANTS FOR '$DB_NAME'@'$DB_HOST';" | grep "$GRANT\|ALL PRIVILEGES" | wc -l` == 0 ]; then
...
mysql_check_add_grant()
{
	...
	if [ "$MYSQL_GRANT_IS_PRESENT" == "1" ]; then
	    ...
	    echo "$DB_HOST" -u root "$DB_NAME" -e "grant $GRANT on *.* to '$DB_USERNAME'@'$DB_HOST'"
		...
		if [ "$MYSQL_GRANT_IS_PRESENT" == "1" ]; then
			...
			report "Failed to add $GRANT grant for MOR DB user. Contact Kolmisoft/MK"  1
			GRANT_IS_PRESENT="1"
      	fi
