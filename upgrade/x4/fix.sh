...
if [ "$mor_branch" != "x4" ]; then
    /usr/bin/mysql -h "$DB_HOST" -u $DB_USERNAME --password=$DB_PASSWORD "$DB_NAME" -e "DELETE FROM conflines WHERE name='PBX_Active' and owner_id='0';"
    /usr/bin/mysql -h "$DB_HOST" -u $DB_USERNAME --password=$DB_PASSWORD "$DB_NAME" -e "INSERT INTO conflines (name,value,owner_id) VALUES ('PBX_Active','1','0');"
    /usr/bin/mysql -h "$DB_HOST" -u $DB_USERNAME --password=$DB_PASSWORD "$DB_NAME" -e "DELETE FROM conflines WHERE name='PG_Active' and owner_id='0';"
    /usr/bin/mysql -h "$DB_HOST" -u $DB_USERNAME --password=$DB_PASSWORD "$DB_NAME" -e "INSERT INTO conflines (name,value,owner_id) VALUES ('PG_Active','1','0');"
fi
